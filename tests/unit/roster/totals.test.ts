...
const fixedGuests: Guest[] = [
  makeGuest({ id: "g1", rsvpStatus: "yes", plusOnesAllowed: 2, side: "bride", groups: ["family"] }),
...
describe("dietaryTotals", () => {
  it("sums each dietary tag once per guest and collects allergy notes verbatim", () => {
    const result = dietaryTotals(fixedGuests);
    expect(result.counts.allergy).toBe(1);
    expect(result.counts.vegan).toBe(1);
    expect(result.counts.none).toBe(4);
...
describe("bySide / byGroup", () => {
  it("groups headcount and responseBreakdown by side", () => {
    ...
    expect(result.groom.headcount.maxAttending).toBe(1 + 1);
